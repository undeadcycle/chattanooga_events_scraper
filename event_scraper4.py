...
def extract_title_and_url(item, config):
    ...
    if url_selector:
        url_tag, url_attrs = next(iter(url_selector.items()), (None, None))
        url_element = item.find(url_tag, **url_attrs) if url_attrs else item.find(url_tag)
        url = url_element.get('href', 'N/A') if url_element else "N/A"
    else:
        url = title_element.get('href', 'N/A') if title_element.name == 'a' else "N/A"
